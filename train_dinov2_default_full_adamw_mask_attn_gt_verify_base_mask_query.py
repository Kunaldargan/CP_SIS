...
from utils import create_endovis_masks, eval_endovis, read_gt_endovis_masks, read_gt_endovis_masks_b
...
def create_binary_masks_pred(predictions, thr=0):
    ...
    for pred in predictions :
        ...
        #if pred["mask_name"] not in segm_masks.keys():
        #    segm  = cv2.imread(osp.join("segms", pred["mask_name"]), 0) / 255
        #    segm  = segm.astype(int)
        #    segm_masks[pred["mask_name"]] = segm
        #else:
        #    segm = segm_masks[pred["mask_name"]]
...
if "18" in dataset_name:
    ...
    num_epochs = 35
    lr = 0.001
    save_dir = "./work_dirs/dinov2_LQ_LORA_ev18_default_full_adamw_Base_in_blocks/"

val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=24, pin_memory=True )
val_dataloader_b = DataLoader(val_dataset_b, batch_size=batch_size, shuffle=False, num_workers=24, pin_memory=True )
val_dataloader_pred = DataLoader(val_dataset_pred, batch_size=batch_size, shuffle=False, num_workers=24, pin_memory=True )
...
extractor = SurgicalDINOCLassifier(
             backbone_size="base", #"large", ##"giant", ##"base", ##base #giant
             r=4, 
             image_shape=(224, 224), 
             decode_type='linear4', 
             lora_layer=None, 
...
if args.checkpoint:
    checkpoint_path  = log_file = osp.join(save_dir, 'checkpoint_epoch_20.pth') ##"checkpoint_epoch_336.pth") #
    ...
    if osp.exists(checkpoint_path):
        ...
        print_log(f"Resuming training from epoch {start_epoch}", log_file)

train_dataset  = Endovis18Dataset_COCO_FULL(data_root_dir = data_root_dir,
                                         mode="train",
                                         vit_mode  = vit_mode,
                                         version   = 0,
...
sample_weights = compute_sample_weights_multilabel(train_dataset)
...
train_dataloader = DataLoader(train_dataset, batch_size=batch_size,  sampler=sampler, num_workers=24, pin_memory=True)

#print(next(iter(val_dataloader)))

# Training loop with resume functionality
for epoch in range(start_epoch, num_epochs):  
    ...
    accumulation_steps = 8  # Effective batch size multiplier (adjust as needed)
    optimiser.zero_grad()  # before starting training
    
    for step, (cnd_imgs, cand_masks, cls_ids, mask_names) in enumerate(tqdm.tqdm(train_dataloader)): 
        
        cls_ids = cls_ids.cuda()

        #candidates
        ###cand_masks.shape: torch.Size([4, 1, 224, 224])
        ###cnd_imgs.shape: torch.Size([4, 3, 224, 224])
        ...
        # compute loss 
        cls_ids = cls_ids - 1  # Shift class indices to be in range [0, 6]
    ...
    average_class_loss = epoch_cls_loss/ len(train_dataloader)
    metric_results = compute_metrics_report(all_preds, all_labels)
    
    
    # Store loss values
    loss_history["class_loss"].append(average_class_loss)
    loss_history["metric_results"].append(metric_results)

    print_log(f"Epoch {epoch}: Class Loss = {average_class_loss:.4f}", log_file)
    print_log(f"Epoch {epoch}: TRAIN Classification report={metric_results}", log_file)
    ...
    with torch.no_grad():
        ...
        for cnd_imgs, cand_masks, cand_mask_1024, _ , mask_names in tqdm.tqdm(val_dataloader_pred): 
    ...
    if epoch % save_interval == 0:
        ...
        }, osp.join(save_dir, f'checkpoint_epoch_{epoch}.pth'))

        print_log(f"Checkpoint saved at Epoch {epoch}", log_file)    
    #"""  
